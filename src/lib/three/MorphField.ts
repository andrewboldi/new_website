...
export function morphField(handle: SceneHandle, opts: Opts = {}) {
  ...
  onFrame((t, dt) => {
    ...
    if (diffOn || i !== lastI || Math.abs(f - lastF) > 0.0008) {
      ...
      // Bond web rebuilds EVERY frame the particles move. This used to run at half
      // rate, which was invisible under the old raw scroll (the shape was jumping
      // anyway). Under the damped scroll the particles glide, so a one-frame-stale
      // web reads as the bonds visibly rubber-banding behind their own endpoints.
      // The O(edges) cost is bounded (~2 edges/particle) and this scene now holds
      // full rate by design (see priorityBoost in MorphBackground.astro).
      updateWeb();
    }
